...
const { KubeBridge, AgentTunnel } = require('./kube-bridge');
...
class EtcdService {
  ...
  async connect(profile) {
    ...
    if (profile.kube && profile.kube.enabled && profile.kube.mode === 'agent') {
      ...
      try {
        ...
        const endpoints = String(profile.endpoints || '')
          .split(',').map((e) => e.trim()).filter(Boolean);
        if (endpoints.length === 0) throw new Error('Enter the in-cluster etcd endpoint(s) to reach through the agent');
  ...
  async clusterOverview() {
    ...
    // Through a port-forward, member client URLs are in-cluster addresses we
    // cannot dial directly. Report status for the member we are tunnelled to
    // (via our own connection) and mark the rest as unprobeable.
    let selfStatus = null;
    if (this.kubeBridge) {
    ...
    for (const m of memberRes.members || []) {
      ...
      if (this.agentTunnel) {
        // The agent can reach every member's in-cluster address directly.
        ...
        try {
          ...
          const s = await probe.maintenance.status();
          member.status = {
            version: s.version,
            ...
            isLeader: String(s.leader) === String(m.ID),
            errors: s.errors || [],
          };
        } catch (err) {
      ...
      if (this.kubeBridge) {
        const isSelf = selfStatus && selfStatus.header &&
