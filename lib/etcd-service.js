...
const { KubeBridge, AgentTunnel, discoverEndpoints, discoverPods } = require('./kube-bridge');
...
class EtcdService {
  ...
  /**
   * Reach every member through the agent and key their status by member id.
   *
   * Members advertise whatever client URL they were configured with, which
   * is not always resolvable from a normal pod — Portworx kvdb advertises
   * *.internal.kvdb. When an advertised address fails, fall back to the IPs
   * of the discovered etcd pods and identify who answered by the member id
   * in the response, since the mapping from pod to member is not knowable
   * up front.
   */
  async _probeMembersViaAgent(members) {
    const found = new Map();
    const tried = new Set();
    ...
    const attempt = async (host, port) => {
      ...
      tried.add(key);
      // An unresolvable name can take the full DNS timeout to fail, so keep
      // this short — these candidates are speculative.
      if (await this.agentTunnel.probe(host, port, MEMBER_PROBE_TIMEOUT_MS) !== 200) return;
      let probe = null;
      try {
        const { localPort } = await this.agentTunnel.openTunnel(host, port);
        probe = this._clientFor(`${scheme}://127.0.0.1:${localPort}`);
        const s = await withTimeout(probe.maintenance.status(), MEMBER_PROBE_TIMEOUT_MS, 'timed out');
        const id = s.header ? String(s.header.member_id) : null;
        if (id && !found.has(id)) found.set(id, s);
      } catch (_) { /* not a usable endpoint */ }
      finally { if (probe) { try { probe.close(); } catch (_) { /* ignore */ } } }
    };

    // Probe concurrently: an unreachable candidate costs a full timeout, and
    // doing that serially across several members is a visible stall.
    const ports = new Set();
    const advertised = [];
    for (const m of members) {
      const url = (m.clientURLs || [])[0];
      if (!url) continue;
      const { host, port } = parseEndpoint(url);
      ports.add(port);
      if (!['127.0.0.1', 'localhost', '::1'].includes(host)) advertised.push([host, port]);
    }
    await Promise.all(advertised.map(([h, p]) => attempt(h, p)));
    if (found.size >= members.length) return found;

    let pods = [];
    try {
      ({ pods } = await discoverPods({
        configPath: this.profile.kube.configPath,
        context: this.profile.kube.context,
        namespace: this.profile.kube.searchNamespace || null,
      }));
    } catch (_) { /* nothing more we can try */ }
    const fallbacks = [];
    for (const pod of pods) {
      if (!pod.ip) continue;
      for (const port of ports) fallbacks.push([pod.ip, port]);
    }
    await Promise.all(fallbacks.map(([h, p]) => attempt(h, p)));
    return found;
  }
  ...
  async clusterOverview() {
    ...
    const viaAgent = this.agentTunnel
      ? await this._probeMembersViaAgent(memberRes.members || [])
      : null;
    ...
    for (const m of memberRes.members || []) {
      ...
      if (this.agentTunnel) {
        const s = viaAgent.get(String(m.ID));
        if (s) {
          member.status = EtcdService._statusOf(s, m.ID);
        } else {
          const endpoint = (m.clientURLs || [])[0];
          member.statusError = endpoint
            ? `the agent could not reach this member at ${endpoint}, and no discovered etcd pod answered as it`
            : 'this member advertises no client URL';
        }
        members.push(member);
        continue;
      }
