 ...
 */
async function discoverEndpoints({ configPath, context, port = 2379, tls = true } = {}) {
  const { pods } = await discoverPods({ configPath, context });
  const scheme = tls ? 'https' : 'http';
  const endpoints = pods
    .filter((p) => p.phase === 'Running' && p.ip)
    .map((p) => `${scheme}://${p.ip}:${port}`);
  if (endpoints.length === 0) {
    throw new Error('No running etcd pods with an IP were found — enter the in-cluster endpoint(s) manually');
  }
  return endpoints;
}

module.exports = {
  KubeBridge, AgentTunnel, listContexts, discoverPods, discoverEndpoints, DEFAULT_KUBECONFIG,
};
